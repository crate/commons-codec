...
def cdc_direct():
    ...
    return DMSTranslatorCrateDB(column_types=column_types, mapping_strategy=ColumnMappingStrategy.DIRECT)
...
def cdc_direct_without_ddl():
    ...
    return DMSTranslatorCrateDB(
        column_types=column_types, mapping_strategy=ColumnMappingStrategy.DIRECT, ignore_ddl=True
    )
...
def test_direct_decode_cdc_update_failure():
    ...
    with pytest.raises(ValueError) as ex:
        DMSTranslatorCrateDB(mapping_strategy=ColumnMappingStrategy.DIRECT).to_sql(MSG_DATA_UPDATE_VALUE)
...
def test_direct_decode_cdc_delete_failure():
    ...
    with pytest.raises(ValueError) as ex:
        DMSTranslatorCrateDB(mapping_strategy=ColumnMappingStrategy.DIRECT).to_sql(MSG_DATA_DELETE)
    assert ex.match("Unable to invoke DML operation without primary key information")
