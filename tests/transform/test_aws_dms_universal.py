...
def cdc_universal():
    ...
    return DMSTranslatorCrateDB(column_types=column_types, mapping_strategy=ColumnMappingStrategy.UNIVERSAL)
...
def test_unknown_strategy(cdc_universal):
    with pytest.raises(TypeError) as ex:
        dtc = DMSTranslatorCrateDB(mapping_strategy="unknown")
        dtc.to_sql(MSG_DATA_LOAD)
    assert ex.match("Column mapping strategy unknown: unknown")
...
def test_universal_decode_cdc_update_failure():
    ...
    with pytest.raises(ValueError) as ex:
        DMSTranslatorCrateDB(mapping_strategy=ColumnMappingStrategy.UNIVERSAL).to_sql(MSG_DATA_UPDATE_VALUE)
...
def test_universal_decode_cdc_delete_failure():
    ...
    with pytest.raises(ValueError) as ex:
        DMSTranslatorCrateDB(mapping_strategy=ColumnMappingStrategy.UNIVERSAL).to_sql(MSG_DATA_DELETE)
    assert ex.match("Unable to invoke DML operation without primary key information")
